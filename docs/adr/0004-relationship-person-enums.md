...
- **Related:** requirements §5.3 (Gender), §7.1 (Couple type), §7.2 (parentage type); CONTEXT.md (Couple, Filiation); ADR 0001 (enum-as-string storage convention), ADR 0002 (write/read seam), ADR 0006 (smart-enum implementation — how these enums are built), ADR 0007 (unknown-representation rule — why Gender uses `null`, not an `Unknown` member). Hardening phase — strengthening the Mini MVP (#58).
...
1. **Storage convention is inherited from ADR 0001, not re-decided.** All three enums are persisted as **strings**, each closed-enum column additionally guarded by a `CHECK (col IN (...))` constraint. Strings are readable/GEDCOM-friendly/reorder-safe; the CHECK recovers database-level rejection of bad values.
   ...
   > **Implementation refined by ADR 0006 (smart-enum convention).** These closed sets are implemented as **smart enums** (a string-valued `sealed` class with `TryParse`), not plain C# `enum`s — same wire/DB contract, but giving a concrete DTO type, RFC 9457 **422** on an unknown value (not a serializer 400), and no `Unknown`/`Invalid` sentinel. Where decisions #2–#4 say "enum," read "smart enum per ADR 0006"; the value sets, nullability, and defaults they specify are unchanged. First implemented for `GenderType` in #103; `CoupleType` (#104) and `ParentageType` (#102) follow.
   >
   > **Whether each enum carries an `Unknown` member is governed by ADR 0007.** Gender uses `null`-as-unknown (decision #3 below) because it is a free-standing optional attribute whose `null` carries no other meaning — ADR 0007's default case.
