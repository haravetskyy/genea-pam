...
1. **Storage convention is inherited from ADR 0001, not re-decided.** All three enums are persisted as **strings**, each closed-enum column additionally guarded by a `CHECK (col IN (...))` constraint. Strings are readable/GEDCOM-friendly/reorder-safe; the CHECK recovers database-level rejection of bad values.

   > **Refinement (implemented in #103, reused by #104):** rather than a plain C# `enum` + `HasConversion<string>()`, these closed sets are implemented as **smart enums** — a `sealed` class with a private constructor, static readonly instances, a `string Value`, an `All` list, and a `TryParse(string?) → T?` that returns `null` on an unrecognized value. This keeps the *exact* same wire and DB contract (a bare string column, the same `CHECK` set) while solving the untrusted-input problem cleanly: the DTO field is the **concrete smart-enum type** (not a vague `string`), `TryParse` funnels an unknown input into inline FluentValidation as an RFC 9457 **422** (not a serializer-level 400), and the domain type carries **no** `Unknown`/`Invalid` sentinel member (the "absent" and "rejected" states live in `null` and in validation, respectively — preserving the no-double-state stance of decisions #3 below and ADR 0003). EF maps it via `HasConversion(v => v.Value, s => TryParse(s)!)`; serialization uses a small `JsonConverter` that reads/writes `Value`, scoped to the type (no global `JsonStringEnumConverter`, so other endpoints are untouched). *Rejected:* (a) plain `int`-backed `enum` — serializes as a number and rejects unknown strings at deserialization with a 400 in the framework's shape, not the project's `errorCode` ProblemDetails; (b) a tolerant converter mapping unknown → an `Invalid` enum member — revives the sentinel/double-state this ADR and ADR 0003 work to avoid. Where decisions #2–#4 say "enum," read "smart enum" per this refinement; the value sets, nullability, and defaults they specify are unchanged.
...
- `CreateCouple` and `AddFiliation` request contracts each gain one optional enum field. `Person` create/update already carry `Gender`; the type changes from `string?` to the `GenderType?` smart enum (decision #1 refinement).
